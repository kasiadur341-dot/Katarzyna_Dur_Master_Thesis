...
# GRCh38 coordinates of LCR22A–LCR22D window
region_chr   <- "22"
...
# ===========================================
# STEP 5: Apply combined filters (alpha = 0.05 AND |log2FC| >= 1)
# ===========================================
alpha   <- 0.05
lfc_thr <- 1

# per-condition significant sets with effect-size threshold
sig_15 <- subset(de_pathway, padj_1.5Mb < alpha & abs(`log2FC_1.5Mb`) >= lfc_thr)
sig_30 <- subset(de_pathway, padj_3.0Mb < alpha & abs(`log2FC_3.0Mb`) >= lfc_thr)

# up/down after combined filtering
...
# For the heatmap, keep genes that pass in **either** condition (ANY)
de_filtered_any <- de_pathway %>%
  dplyr::filter( (padj_1.5Mb < alpha & abs(`log2FC_1.5Mb`) >= lfc_thr) |
                   (padj_3.0Mb < alpha & abs(`log2FC_3.0Mb`) >= lfc_thr) )

# ===========================================
# STEP 6: Build matrix for Heatmap (auto-detect all log2FC_* columns)
# ===========================================
log2_cols <- grep("^log2FC", names(de_filtered_any), value = TRUE)

fc_matrix <- de_filtered_any |>
...
pheatmap(
  ...
  main = "Expression of 22q11.2 Genes (padj < 0.05 & |log2FC| ≥ 1)",
  fontsize_row = 10,
  fontsize_col = 12,
  legend = TRUE,
  angle_col = 45,
...
# ===========================================
# STEP 7: Bar plot of significant Up/Down (same combined filters)
# ===========================================
counts <- list(
  `1.5Mb` = data.frame(
    Condition = "1.5Mb",
...
p <- ggplot(counts_long, aes(x = Condition, y = Count, fill = Direction)) +
  ...
  geom_text(aes(label = Count),
            ...
            vjust = -0.6, size = 6) +
  labs(
    title = "Significant Up/Down-Regulated Genes (padj < 0.05 & |log2FC| ≥ 1)",
...
ggsave("barplot_22q11_sig.pdf", p, width = 8, height = 6)

cat("Saved:\n - heatmap_22q11_sig.pdf\n - barplot_22q11_fixed.pdf\n")

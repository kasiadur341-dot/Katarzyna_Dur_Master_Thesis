...
titled_plot <- ggdraw() +
  draw_label("Top 10 Up- and Down-regulated Genes – NPCs (3.0Mb vs Control)",
...
go_plot <- print(ggplot(top_go, aes(x = term_name_wrapped, y = -log10(p_value))) +
  ...
  coord_flip() +
  enrich_theme +
  labs(title = "Top 10 GO:BP Terms – NPCs (3.0Mb vs Control)",
       x = "GO Biological Process", y = "-log10(p-value)"))

ggsave("GO_BP_Top10_3.0Mb_vs_Control.pdf", go_plot, width = 11.7, height = 8.3)
...
pathway_plot <- print(ggplot(top_pathways, aes(x = term_name_wrapped, y = -log10(p_value),
  ...
  coord_flip() +
  enrich_theme +
  labs(title = "Top 20 Enriched Pathways – NPCs (3.0Mb vs Control)",
       x = "Pathway / Term", y = "-log10(p-value)",
       size = "Gene Count", color = "Source"))

ggsave("Enriched_Pathways_Top20_3.0Mb_vs_Control.pdf", pathway_plot, width = 11.7, height = 8.3)

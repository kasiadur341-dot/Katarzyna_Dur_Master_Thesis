...
# Filter metadata for NPCs (celltype 'p') from 1.5 carriers and control
npc_meta <- subset(meta, Celltype == "p" & Patient %in% c("d8", "d9", "c11"))
...
# Save tables
write.csv(sig_p_npc, "npc_sig_pval.csv", row.names = FALSE)
write.csv(sig_padj_npc, "npc_sig_adj_pval.csv", row.names = FALSE)
write.csv(sig_up_npc, "npc_sig_upregulated.csv", row.names = FALSE)
write.csv(sig_down_npc, "npc_sig_downregulated.csv", row.names = FALSE)

# Create summary row and add to summary list
summary_row <- data.frame(
  Comparison = "NPC_1.5Mb_vs_Control",
...
summary_list[["NPC_1.5Mb_vs_Control"]] <- summary_row
...
# Identify the most extreme gene
outlier <- res_df[which.max(-log10(res_df$pvalue)), ]

# Volcano plot 
plot(res_df$log2FoldChange, -log10(res_df$pvalue),
     pch = 20, col = "gray",
     main = "Volcano Plot − NPCs",
...
ggsave("VolcanoPlot_NPCs_clean.pdf", p, width = 7.05, height = 5, units = "in")

# Boxplot: Raw Counts
boxplot(log(npc_counts_filtered + 1),
        main = "Raw log−counts (Collapsed) - NPCs",
        ylab = "log1p(Counts)",
        las = 2, col = "gray")
...
boxplot(log(norm_counts + 1),
        main = "Normalized log−counts − NPCs",
...
pheatmap(
  ...
  main = "Heatmap of Top DE Genes — NPCs (VST expression, log2-like scale, capped at 10)",
  border_color = NA,
  use_raster = TRUE,
  filename = "NPCs_heatmap.pdf",
  width = 8, height = 10
)
...
autoplot(pca, data = data.frame(group = group_info), colour = 'group', label = TRUE, label.size = 3) +
  ...
  ggtitle("PCA − NPCs (1.5Mb vs Control)") +
  theme_minimal() +
...
p_dend <- ggplot() +
  ...
  labs(title = "Sample Clustering Dendrogram – NPCs",
       x = NULL, y = "Height") +
  scale_y_continuous(
    limits = c(0, y_top),
...
ggsave("NPCs_dendrogram.pdf", plot = p_dend, width = 8, height = 6, units = "in")

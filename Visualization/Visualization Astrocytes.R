...
groups <- unique(astro_meta$Group)
collapsed_counts <- sapply(groups, function(g) rowSums(astro_counts[, astro_meta$Row.names[astro_meta$Group == g], drop = FALSE]))
...
# DESeq2
dds <- DESeqDataSetFromMatrix(astro_counts_filtered, astro_meta_collapsed, design = ~ Sex + Disease)
dds <- DESeq(dds)
...
write.csv(all_enrich, "Astrocytes_all_enrichment_combined.csv", row.names = FALSE)

...
# Filter metadata for astrocytes (celltype 'a') from 1.5 carriers and control
...
# Load shrinkage package (only once)
...
# Save tables
write.csv(sig_p_astro, "astro_sig_pval.csv", row.names = FALSE)
write.csv(sig_padj_astro, "astro_sig_adj_pval.csv", row.names = FALSE)
write.csv(sig_up_astro, "astro_sig_upregulated.csv", row.names = FALSE)
write.csv(sig_down_astro, "astro_sig_downregulated.csv", row.names = FALSE)

# Gene count summary
cat("Gene count summary for astrocytes (1.5 vs control):\n")
cat("1. Raw p-value < 0.05: ", nrow(sig_p_astro), "genes\n")
cat("2. Adjusted p-value < 0.05: ", nrow(sig_padj_astro), "genes\n")
cat("3. Upregulated (log2FC > 1 & adj.p < 0.05): ", nrow(sig_up_astro), "genes\n")
cat("4. Downregulated (log2FC < -1 & adj.p < 0.05): ", nrow(sig_down_astro), "genes\n")

# Create summary row and add to summary list
summary_row <- data.frame(
  Comparison = "Astrocytes_1.5Mb_vs_Control",
  Significant_padj_0.05 = nrow(sig_padj_astro),
  Upregulated_log2FC_gt1 = nrow(sig_up_astro),
  Downregulated_log2FC_lt_neg1 = nrow(sig_down_astro)
)

# Initialize or append to global summary list
if (!exists("summary_list")) summary_list <- list()
summary_list[["Astrocytes_1.5Mb_vs_Control"]] <- summary_row

# Volcano plot
plot(res_df$log2FoldChange, -log10(res_df$pvalue),
     pch = 20, col = "gray",
     main = "Volcano Plot − Astrocytes",
...
# Boxplot Raw Counts
boxplot(log(astro_counts_filtered + 1),
        main = "Raw log−counts (Collapsed) − Astrocytes",
        ylab = "log1p(Counts)",
        las = 2, col = "gray")
...
boxplot(log(norm_counts + 1),
        main = "Normalized log−counts − Astrocytes",
...
annotation_col <- data.frame(Group = astro_meta_collapsed$Disease)
rownames(annotation_col) <- rownames(astro_meta_collapsed)
...
pheatmap(
  ...
  main = "Heatmap of Top DE Genes — Astrocytes (VST expression, log2-like scale, capped at 10)",
  border_color = NA,
  use_raster = TRUE,
  filename = "Astrocytes_heatmap.pdf",
  width = 8, height = 10
)
...
group_info <- astro_meta_collapsed$Disease[match(rownames(t(norm_counts)), rownames(astro_meta_collapsed))]
...
autoplot(pca, data = data.frame(group = group_info), colour = 'group', label = TRUE, label.size = 3) +
  ...
  ggtitle("PCA − Astrocytes (1.5Mb vs Control)") +
  theme_minimal() +
...
p_dend <- ggplot() +
  ...
  labs(title = "Sample Clustering Dendrogram – Astrocytes",
       x = NULL, y = "Height") +
  scale_y_continuous(
    limits = c(0, y_top),
...
ggsave("Astrocytes_dendrogram.pdf", plot = p_dend, width = 8, height = 6, units = "in")

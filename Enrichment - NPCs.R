...
# Filter metadata for NPCs: 1.5Mb carriers (d8, d9) vs control (c11)
...
sig_up_npc <- subset(res_df, padj < 0.05 & log2FoldChange > 0.56)
sig_down_npc <- subset(res_df, padj < 0.05 & log2FoldChange < -0.56)
...
titled_plot <- ggdraw() +
  draw_label("Top 10 Up- and Down-regulated Genes – NPCs (1.5Mb vs control)",
             fontface = 'bold', size = 16, x = 0.5, y = 0.98, hjust = 0.5) +
  draw_plot(final_plot, y = 0, height = 0.95)

print(final_plot)
ggsave("Top10_NPC_DEG_barplots_NPCs_1.5Mb_vs_control_A4.pdf", titled_plot, width = 11.7, height = 8.3)
...
go_plot <- print(ggplot(top_go, aes(x = term_name_wrapped, y = -log10(p_value))) +
  ...
  coord_flip() +
  enrich_theme +
  labs(title = "Top 10 GO:BP Terms – NPCs (1.5Mb vs control)",
       x = "GO Biological Process", y = "-log10(p-value)"))

ggsave("GO_BP_Top10_NPCs_1.5Mb_vs_control.pdf", go_plot, width = 11.7, height = 8.3)
...
pathway_plot <- print(ggplot(top_pathways, aes(x = term_name_wrapped, y = -log10(p_value),
  ...
  coord_flip() +
  enrich_theme +
  labs(title = "Top 20 Enriched Pathways – NPCs (1.5Mb vs control)",
       x = "Pathway / Term", y = "-log10(p-value)", size = "Gene Count", color = "Source"))

ggsave("Enriched_Pathways_Top20_NPCs_1.5Mb_vs_control.pdf", pathway_plot, width = 11.7, height = 8.3)
